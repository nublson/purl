...
export function useChatConversation() {
  ...
  const pendingMessageHandledKeyRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (!pendingMessage) {
      pendingMessageHandledKeyRef.current = null;
      return;
    }
    const dedupeKey = `${pendingMessage.text}:${pendingMessage.mentions.map((m) => m.id).join(",")}`;
    if (pendingMessageInFlightRef.current) return;
    if (pendingMessageHandledKeyRef.current === dedupeKey) return;

    pendingMessageInFlightRef.current = true;
    pendingMessageHandledKeyRef.current = dedupeKey;
    const { text, mentions: pendingMentions } = pendingMessage;
    clearPendingMessage();
    ...
    void (async () => {
      try {
        ...
        const mentionedLinkIds = pendingMentions.map((m) => m.id);
        const requestId = newRequestId();
        lastRequestIdRef.current = requestId;
        sendMessage(
          { text },
        ...
        setMessageMentions((prev) => [...prev, [...pendingMentions]]);
        clearMentions();
      } finally {
  ...
  const handleDeleteChat = useCallback(async (): Promise<boolean> => {
    const id = chatIdRef.current;
    if (!id) return false;
