...
- New pages under `src/app/(public)/` still need a `publicRoutes` entry in `src/proxy.ts` — the `(public)` route group alone does not bypass auth middleware.
- `publicRoutes` in `src/proxy.ts` supports `match: "exact" | "prefix"` (e.g. `/.well-known` and `/api/auth` use prefix matching). Routes with `whenAuthenticated: "next"` skip the session lookup entirely; only `"redirect"` routes (`/login`, `/signup`) resolve the session.
